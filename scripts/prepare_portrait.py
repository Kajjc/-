...
2. Кадрирование "голова и плечи" под слот портрета в диалоге (~1.35:1, см.
   ...
   получалось бы ~30 px шириной и растянутым по горизонтали.
...
SLOT_ASPECT = 140.8 / 104.0   # ширина/высота слота портрета в диалоге
...
def prepare(path, out_path):
    ...
    crop_w = int(round(crop_h * SLOT_ASPECT))
