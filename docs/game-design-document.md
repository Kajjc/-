...
## 4. Контент — три домена, четыре сюжета, 12 сценариев
...
| **Закупки** — переговоры с поставщиком | закупщик | менеджер поставщика | зеркально к продажам — извлечение уступок, а не защита цены; та же механика движка, разные роли | от «мы партнёры» до защитного |

Второй сюжет HR переиспользует портрет/фон первого (тот же руководитель, та же
сфера) — в коде портрет и фон привязаны к **домену**, а не к конкретному файлу
сценария, поэтому не нужно рисовать отдельные ассеты под каждую пару
сюжет×сложность.

**12 файлов** = 4 сюжета × 3 уровня сложности (`_easy`/`_medium`/`_hard`,
...
50, medium до 150, hard до 300 (было 6-8/8-10/10-11). Фактически достигнуто: easy
50/50 везде (100%), medium 141-150/150 (94-100%), hard 167-179/300 (56-60% —
честный, принятый недобор, предел ручного написания диалога за один заход без
потери качества реплик; всё равно ~15-17-кратный рост от исходной глубины). Файлы
теперь 78-266 узлов каждый (было 15-30) — длинный «хребет» последовательных фаз
разговора с короткими ответвлениями-развилками для слабых ответов, реконвергирующими
вперёд, а не экспоненциальное ветвление. На hard win теперь единообразно требует все
три навыка ≥2 одновременно во всех четырёх сюжетах. Точные цифры по каждому файлу —
`docs/eval-rubric.md` §5.
...
- ✅ 12 сценариев (4 сюжета × 3 сложности) в 3 доменах, честная и проверенная
  прогрессия сложности. Глубина (19-20.09) увеличена до цели 50/150/300 ходов —
  фактически 50/50, 141-150/150, 167-179/300 (см. раздел 4).
...
- ✅ Гейты по навыкам распределены по всей длине разговора (27-40% узлов), не
  только у финала.
...
- ⬬ **Добить hard-файлы всех четырёх сюжетов ближе к 300 ходов** (сейчас 167-179,
  56-60% от цели) — решение о том, стоит ли ещё один заход, отложено пользователем.
  Приём («хирургическая вставка» под-эпизодов перед узлом-потолком) уже отработан
  и легко повторим ещё 2-3 раза на каждый файл.
...
- ⬜ **`minCumulativeScore`** (`docs/eval-rubric.md` §4, вариант 2) — общий,
  ...
  «правильный» путь в один узел) хватило на все 12 сценариев, дырок не найдено. Идея
