...
- **HR**: интересы vs позиции → `state_interest`/`position_push`; объективные критерии →
  ...
  покрыто, соответствует уже существующему `test_salary_review.json`.
...
|---|---|---|
| **+2** | Продвинутое/составное применение техники. Как правило, гейтится `requiredSkills` уровня ≥2 (недоступно игроку со слабым навыком) — сравни с уже фактическим `anchor_with_flex`/`package_deal` в `test_salary_review.json`, у которых `points: 2` и требование навыка ≥2. | Обычно ведёт к лучшей из доступных на этом узле веток (`end_win` или её аналог в других доменах). |
...
диапазона автор сценария выбирает по степени тяжести конкретной реплики. Рабочий пример уже
есть в `test_salary_review.json`: `position_push` в узле `n1` («Мне нужно больше денег, иначе я
уйду») стоит `-1` — это мягкое, ещё не враждебное повторение позиции; тот же тег в узле `n3c`
(«Тогда зачем вообще стараться») стоит `-2` — это уже самообесценивающая, а не просто
позиционная реплика. Это не ошибка/несогласованность разметки, а корректный пример того, как
работает диапазон: значение внутри диапазона отражает тяжесть конкретной формулировки, тег
остаётся тем же.
...
**Уточнение — один конкретный случай, где нужна защита:** при разборе `test_salary_review.json`
найдена реальная дыра такого типа. Путь `n1 → position_push(-1) → n2b → de_escalate(+1) → n3`
...
2. **Общий, на уровне движка (рекомендуется как v2-улучшение, не блокирует хакатон):** добавить
   ...
   §1), умноженное на `+1`, и масштабируется `meta.difficulty` (см. §5). При недостаточном
   счёте опция должна не исчезать молча, а быть видимой, но заблокированной — как уже сделано
   для `requiredSkills` (`GetLockLabel`), чтобы игрок понимал, что ему не хватило суммарной
   техники, а не конкретного навыка. **Это требует изменения кода** (`DialogueModels.cs` — новое
   поле; `DialogueEngine.cs` — проверка в `IsOptionAvailable`/`GetLockLabel`) — вне зоны
   ответственности этого документа, фиксирую здесь как явную рекомендацию тому, кто владеет
   движком, и отдельно — сценарному дизайнеру, чтобы для новых сценариев сразу закладывать
   опцию 1 (не сводить обходной путь и «правильный» путь в один и тот же узел с одинаковым
   набором лучших опций).

---

## 5. Калибровка под сложность (`meta.difficulty`)

В коде `difficulty` — произвольный `int` без задокументированной шкалы (используется только для
подбора ближайшего сценария в `ScenarioLibrary.Pick`). Для рубрики фиксирую предлагаемую шкалу
**1–5** (согласовать с `gamification-ux-designer`/`scenario-designer`, если у них уже есть иное
соглашение) и правило, как под неё калибруется разметка баллов и гейтов:

| Параметр | Difficulty 1–2 (низкая) | Difficulty 3 (средняя) | Difficulty 4–5 (высокая) |
|---|---|---|---|
| Гейт для `+2`-опций (`requiredSkills`) | уровень ≥2 («умеренно») | уровень ≥2, иногда ≥3 | уровень ≥3 («сильно») по центральной для домена технике |
| Выбор внутри диапазона `points` для ошибок | ближе к верхней границе (мягче: `position_push`≈−1, `escalate` может быть смягчена до одного шага предупреждения перед `-2`) | середина диапазона | ближе к нижней границе (жёстче: одна `personal_attack`/`escalate` сразу `-2` и обычно сразу к `end_fail`) |
| «Запас на восстановление» (число промежуточных узлов вида `*b`/`*c`, дающих второй шанс после ошибки) | 1–2 узла восстановления на ошибку | 1 узел восстановления | обычно 0 — ошибка на central-технике сразу закрывает путь к `end_win`, оставляя максимум `end_compromise` |
| Требование к `end_win` | одна `+2` реплика на центральной технике может быть достаточна | нужна связка из 2 разных central-техник с `points > 0` | нужна связка из ≥2 central-техник **и** отсутствие ни одной `-2` реплики за весь диалог |
| Тон оппонента на старте | нейтральный | нейтральный→скептический | скептический/защитный/напористый с первой реплики (см. тона в `negotiation-domains.md`) |
| Если реализован `minCumulativeScore` (§4, п.2) | порог = (кол-во central-техник домена) × 1 | × 1.5 (округление вверх) | × 2 |
