...
- ✅ Три полноценных сценария (HR / продажи / закупки) с ветвлением, проверены на дыры в логике исхода.
...
### 17.09 (сегодня)
- ⬜ **Публичный репозиторий**: решить видимость (public/private) и запушить — нужно подтверждение перед первым `git push`, т.к. это действие с видимыми последствиями. Единственный оставшийся пункт на сегодня.
- 🔶 **Пробная WebGL-сборка** — изначально стояла на 20-21.09, поднята на сегодня по итогам прогона `demo-reliability-checker` (см. риски ниже): нужно рано проверить, не теряется ли кириллица в легаси-шрифте на WebGL. В процессе — сцена `Assets/Scenes/Main.unity` уже сохранена, добавляется в Build Settings.
