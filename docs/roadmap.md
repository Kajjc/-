...
- ✅ **Пробная WebGL-сборка**: найдено и исправлено 17.09 — `EditorBuildSettings.asset` фактически был пуст (`m_Scenes: []`), несмотря на то что сцена `Main.unity` была сохранена; сцена добавлена в Build Settings. Сборка через `Assets/Editor/BuildScript.cs` (`-executeMethod Arena.EditorTools.BuildScript.BuildWebGL`) прошла успешно (build report, 29.0 MB, без ошибок компиляции/сборки). Билд лежит локально в `unity/WebGL-Build/` (в `.gitignore`, не коммитится). **Не закрыто до конца**: успешная сборка не доказывает, что кириллица реально видна в браузере — само нельзя увидеть без открытия `index.html` через локальный сервер (WebGL не грузится с `file://`) и визуальной проверки; это стоит сделать до публикации ссылки жюри, ещё не сделано.
...
- **Кириллица в WebGL (высокий приоритет, находка `demo-reliability-checker` от 17.09)**: весь текст — на легаси `UnityEngine.UI.Text` с `LegacyRuntime.ttf`, весь контент на русском. Пробная сборка 17.09 прошла успешно (без ошибок), но это не подтверждает, что глифы реально рендерятся в браузере — нужна отдельная визуальная проверка (`unity/WebGL-Build/index.html` через локальный http-сервер, не `file://`), ещё не проведена. Если оппонент увидит "тофу"-квадраты вместо кириллицы — мигрировать на TextMeshPro (уже доступен в проекте) с самодельным SDF-атласом на кириллическом шрифте (PT Sans / Inter / Roboto / Noto Sans — обсуждались как кандидаты), т.к. дефолтный TMP-ассет LiberationSans SDF кириллицу не поддерживает.
