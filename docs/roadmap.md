...
- **Кириллица в WebGL — ПОДТВЕРЖДЕНО СЛОМАНО (18.09)**: визуальная проверка проведена (локальный http-сервер с gzip-заголовками + headless-скриншот Edge после полной загрузки) — экран показывает только цифры/пунктуацию, ни одной кириллической буквы не рендерится (`LegacyRuntime.ttf` в WebGL-билде без кириллических глифов). Весь контент продукта на русском — билд в текущем виде непригоден для показа жюри. **Следующий шаг — миграция на TextMeshPro** (уже доступен в проекте) с самодельным SDF-атласом на кириллическом шрифте (кандидаты: PT Sans / Inter / Roboto / Noto Sans — дефолтный TMP-ассет LiberationSans SDF кириллицу не поддерживает). См. К1 в `docs/feature-hypotheses.md`.
