...
- **Типичная сложность/тон**: базовый–средний уровень, тон нейтральный →
  раздражённый при эскалации (уже отражено в `test_salary_review.json`).
...
- **Статус**: уже есть черновой тестовый сценарий движка
  (`unity/Assets/Resources/Scenarios/test_salary_review.json`) — он подтверждает
  формат и механику, но структуру/развилки нужно углубить под финальные критерии
  (добавить требование сочетания навыков, не только отдельных порогов). Тема
  входит в финальную тройку, тестовый файл берём за основу, а не пишем с нуля.
